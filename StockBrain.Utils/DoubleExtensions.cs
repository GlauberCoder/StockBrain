...
{
    public static double ToPrecision(this double number, int precision = 2) => double.IsNaN(number) || double.IsInfinity(number) ? 0 : (double)Math.Round((decimal)number, precision);
    public static double ToPercentage(this double number, int precision = 0) => double.IsNaN(number) || double.IsInfinity(number) ? 0 : (double)Math.Round((decimal)number * 100, precision);
